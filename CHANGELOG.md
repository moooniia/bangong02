...
## v0.12.31 / BUILD 2026.06.30.2（2026-06-30）

- Fixed: 修复 PDF 编辑器中页面旋转后导出统一 A4/A3 或原尺寸时，页面横竖方向仍按源页面判断的问题。现在先按源页面可见宽高判断预览方向，再叠加用户旋转角度，最终用旋转后的方向决定导出页面形状，避免横向内容塞进竖版页面或竖向内容塞进横版页面造成异常白边。
...
fix: Word翻译遇到老版.doc或损坏文件时给出明确提示，不再泄露python-docx内部报错；.doc重命名的真docx也走原地保格式翻译
...
feat: 文件翻译Word转Word原地保留格式（段落样式/字体/表格/图片），不再用提取纯文字重建空文档的方式
