...
print("pages", len(details), "wm_count", len(wm))
for i, page in enumerate(details):
    ...
    tq = round(_page_table_quality(page), 3) if has_tbl else None
    mainly = _page_mainly_table(page) if has_tbl else False
    sample = []
    for b in page.get("textblocks") or []:
        t = (b.get("text") or "").strip()
        if t and len(sample) < 2:
            sample.append(t[:60])
    print(
        f"p{i+1}: sig={sig} wm={wm_heavy} mlen={mlen} edit={editable} snap={snap} "
        f"tbl={has_tbl} tq={tq} mainly={mainly} sample={sample}"
    )